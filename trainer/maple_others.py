...
class MultiModalPromptLearner(nn.Module):
    ...
    def forward(self,maefeatures):
        ...
        bias = self.meta_net(maefeatures)  # (batch, ctx_dim)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # print('loading remote MAE VITAE model')
    ...
    def forward(self, image, label=None):
        ...
        # image1 = torch.rot90(image,1,[2,3])
        # image2 = torch.rot90(image,2,[2,3])
        # image3 = torch.rot90(image,3,[2,3])
        # image_features1 = self.image_encoder(image.type(self.dtype), shared_ctx, deep_compound_prompts_vision)
        # image_features2 = self.image_encoder(image.type(self.dtype), shared_ctx, deep_compound_prompts_vision)
        # image_features3 = self.image_encoder(image.type(self.dtype), shared_ctx, deep_compound_prompts_vision)
        ...
        # logits2 = []
        # for pts_i, imf_i in zip(prompts, image_features2):
        #     text_features = self.text_encoder(pts_i, tokenized_prompts,deep_compound_prompts_text)
        #     text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        #     l_i = logit_scale * imf_i @ text_features.t()
        #     logits2.append(l_i)
        ...
        # logits3 = []
        # for pts_i, imf_i in zip(prompts, image_features3):
        #     text_features = self.text_encoder(pts_i, tokenized_prompts,deep_compound_prompts_text)
        #     text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        #     l_i = logit_scale * imf_i @ text_features.t()
        #     logits3.append(l_i)
        ...
        image_features_bias = remote_mae_feature.type(self.dtype)
  
        # mean = torch.mean(remote_mae_feature)
        # # mean = torch.mean(image_features)
        # var = torch.var(remote_mae_feature)
        # std = torch.sqrt(var)
        # a = torch.normal(mean, std, size=image_features.shape).type(self.dtype)
        # # image_features_bias = torch.randn(image_features.shape).type(self.dtype)
        image_features = image_features +image_features_bias
        # image_features = image_features + a.cuda()
        ...
        # logits = (logits + logits1 + logits2 + logits3)/4
        if self.prompt_learner.training:
            # mse1 = F.mse_loss(image_features,image_features1,reduction='mean')
            # mse2 = F.mse_loss(image_features,image_features2,reduction='mean')
            # mse3 = F.mse_loss(image_features,image_features3,reduction='mean')
            # mse_all = (mse1 + mse2 +mse3)/3.0

            # mse_all = F.cross_entropy(logits1, label) + F.cross_entropy(logits2, label) +F.cross_entropy(logits3, label)+F.cross_entropy(logits, label)
            # return mse_all/4
