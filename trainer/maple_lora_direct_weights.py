...
class MultiModalPromptLearner(nn.Module):
    ...
    def forward(self,maefeatures):
        ...
        noise = torch.randn(maefeatures.shape).cuda().type(self.dtype)*mean
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if lora_layer:
            ...
            self.lora_layer = list(range(len(clip_model.transformer.resblocks)))
        # create for storage, then we can init them or load weights
        self.w_As = nn.ParameterList()
        self.w_Bs =  nn.ParameterList()

        self.w_As1 =  nn.ParameterList()
        self.w_Bs1 =  nn.ParameterList()
        ...
        for t_layer_i, blk in enumerate(clip_model.transformer.resblocks):
            ...
            w_a_linear_q = Parameter(torch.empty((self.dim2, r)),requires_grad=True)
            w_b_linear_q = Parameter(torch.empty((r, self.dim2)),requires_grad=True)
            w_a_linear_v = Parameter(torch.empty((self.dim2, r)),requires_grad=True)
            w_b_linear_v = Parameter(torch.empty((r, self.dim2 )),requires_grad=True)
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            if name_to_update not in name:
                ...
                if "VPT" in name or 'w_As' in name  or 'w_As' in name:
